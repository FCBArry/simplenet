...
logging.basicConfig(level=logging.INFO,filename='csv.log',filemode='w',format= '%(asctime)s-%(name)s-%(levelname)s-%(message)s')
...
def process_excel(filepath, outpath):
    ...
    for i in range(4, n):
        ...
        for j in range(len(rowData)):
            ...
            if (is_int_array_type(attrubiteTypeDic[j])):
                strSource = str(rowData[j]).split(',')
                strTarget = ''
                for k in range(len(strSource)):
                    strTarget += str(int(float(strSource[k]))) + ','
...
def is_int_array_type(dataType):
    return dataType.startswith('vector<int>')
